...
function getChina(provinces){
  ...
  if(newProvinces[0]!= '' && newProvinces[1] != ''){
    ...
    let province2 = [{ name: "北京", cities: ["西城", "东城", "崇文", "宣武", "朝阳", "海淀", "丰台", "石景山", "门头沟", "房山", "通州", "顺义", "大兴", "昌平", "平谷", "怀柔", "密云", "延庆"] },
    ...
    for (let i = 0; i < province.length; i++){
        if (a == province[i].name) {
           name = province2[i].name;
        }
        for (let u=0; u < province[i].cities.length; u++) {
          if (city == province[i].cities[u]) {
              cities = province2[i].cities[u];
              }
        }
    }
    return `${name}.${cities}`;
