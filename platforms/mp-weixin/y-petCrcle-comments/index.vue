<template >
  <view class="content">
    <view class="PetCircle">
      ...
      <view class="Pet_img">
        <view
          v-for="(item, index) in user.DuaminImg"
          :key="index"
          class="P_main"
        >
          <u-lazy-load
    ...
    <view class="" style="flex: 1">
      <view class="comments"> 评论列表 </view>
      <!-- 总体评论 -->
      <view class="c_mian pD">
        <view
          class="c_name"
          v-for="(ite, ide) in commentData"
          :key="ide"
          @tap="PopUp(ide)"
        >
          <!-- 头像 -->
          ...
          <view class="avatar">
            ...
            <!-- 回复 -->
            <view class="huifu">{{ite.num}}条回复 ></view>
          </view>
        </view>
      </view>
    </view>
...
export default {
  ...
  methods: {
    ...
    // 进入评论详情方法
    PopUp(i) {
      let commentData = encodeURIComponent(JSON.stringify(this.commentData[i]));
      console.log(commentData);
      uni.navigateTo({
        url: "../y-petCrcle-reply/index?id=" + commentData,
      });
