      ...
      this.IfRegistered() //请求判断用户是否注册过
        .then(res => {  
          ...
          if (res.desc === '用户已经存在') { //就不需要注册了 要调用按钮 授权新信息
            ...
            this.info.UserType = res.data[0].UserType; //用户类型

            if(Object.keys(uni.getStorageSync('UserInfo')).length > 0){ //本地有数据
              this.navigateToRoles();; //跳转到对应页面                 
            }else
            {//本地没有数据
    ...
    methods: {
      ...
      navigateToRoles() { //跳转对应详情页面
        if (this.info.UserType === 'user') {
          console.warn('开始渲染用户列表')
          uni.switchTab({url: '/platforms/mp-weixin/y-home/index',})
        } else if(this.info.UserType === 'merchant') {
          console.warn('开始渲染列表商家')
